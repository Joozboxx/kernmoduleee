...
public class OrbitCamera : MonoBehaviour
{
    ...
    public float camSmoothingTarget = 0.1f;
    ...
    [SerializeField] private Vector3 targetOffset = new Vector3(0.8f, 1.8f, -3.7f);
    [Space]
    [SerializeField] private Transform camShaker;
    ...
    void Target()
    {
        ...
        cameraTransform.rotation = Quaternion.Slerp(transform.rotation, rotation, Time.deltaTime * 10f);

        Vector3 target = viewTarget.transform.position + (viewTarget.transform.right * targetOffset.x) + viewTarget.transform.forward * targetOffset.z + Vector3.up * targetOffset.y;
        cameraTransform.position = Vector3.Lerp(cameraTransform.position, target, camSmoothingTarget);
    }
    ...
    void NoTarget()
    {
        ...
        cameraTransform.position = Vector3.Lerp(cameraTransform.position, (newPosition + camShaker.localPosition + cameraOffset), camSmoothing);
        //cameraTransform.position = newPosition + camShaker.localPosition + cameraOffset;
        cameraTransform.rotation = smoothRotation * camShaker.localRotation;
    }
